...
In RStudio, typing <kbd>Alt</kbd> + <kbd>-</kbd> will write ` <- ` in a single keystroke on a PC (on a Mac, use <kbd>Option</kbd> instead).
...
collaborators. In RStudio, best select your code, press
<kbd>Ctrl</kbd> + <kbd>Shift</kbd> + <kbd>A</kbd> or go into the "Magic Wand"- /
"Code Tools"-menu and click "Reformat Code".
...
functions `is.na()`, esp. its negation `!is.na()`:
