...
To select columns of our `surveys` data frame, use `select()`. The first argument
to this function is the data frame (`surveys`), and the subsequent
arguments are the columns to keep.

```{r, results = 'hide', purl = FALSE}
select(surveys, plot_type, species, weight)
```
...
```{r, results = 'hide', purl = FALSE}
select(surveys, -plot_id, -record_id, -species_id)
```

This will remove the (primarily machine-readable) `id` variables.
...
What if you want to select and filter at the same time? There are several
ways to do this.
...
(small_animals <- select(small_animals, species_id, sex, weight))
```

This is readable, but can clutter up your workspace with lots of objects that you have to name individually. With multiple steps, that can be hard to keep track of.

*Pipes* are better, because they let you take
the output of one function and send it directly to the next, which is useful
when you need to do many things to the same dataset.  Pipes in R look like
`%>%` and are made available via the **`magrittr`** package, installed automatically
with **`dplyr`**. If you use RStudio, you can type the pipe with <kbd>Ctrl</kbd>
+ <kbd>Shift</kbd> + <kbd>M</kbd> on Windows and with <kbd>Cmd</kbd> + 
<kbd>Shift</kbd> + <kbd>M</kbd> on macOS.
...
Note that the final data frame is the top-left-most part of this expression.
...
Mutate is also useful to change character strings. E.g. spelling `F` and `M` out:

```{r mutate-abbr}
surveys %>%
  mutate(sex = if_else(is.na(sex), true = "undetermined", 
                       false = if_else(sex == "F", "female", "male")
                       )
         )
```

Note that this time, we are overwriting the data, because we give `mutate` the
same target colum as `if_else`'s source column.

`if_else` is similar to the "conditional subsetting" we learned earlier:
Its first argument is a condition (here using `==`) that returns a logical result
(`TRUE` or `FALSE`), that determines which of the following two arguments (`true = ...` or `false = ...`) is used as the function's output. Because we need to check
3 different cases, we need to next 2 `if_else` calls.

Changing the raw data in this way can be justified because:

1. it's explicitly coded
2. the purpose is to make the data more self-explanatory.

The latter will be useful later when we draw diagrams from this data.
...
then combine the results. **`dplyr`** supports this with the `group_by()` function.
...
for each value or combination of values. For this task, **`dplyr`** provides
...
The `count()` function is shorthand for something we've already seen: grouping by a variable, and summarizing it by counting the number of observations in that group. In other words, the above is equivalent to:  
...
for *one* variable (i.e., `sex`). 
If we wanted to count *combination of variables*, such as `sex` and `species`, 
we would specify the first and the second variable as the arguments of `count()`:
...
For instance, we might want to arrange the table above in two ways:

```{r purl = FALSE}
surveys %>%
  count(sex, species) %>%
  arrange(species,  # first by alphabetical order of the levels
          desc(n)   # then by descending order of the count.
  )
```
...
> # or
> args(count)
> # show that "n" is the default name of the column that is added by that function
> ```
>
...
> ```
>
> 3. Use `group_by()` and `summarize()` to find the mean, min, and max hindfoot
...
> ```
>
> 4. What was the heaviest animal measured in each year? Return the columns `year`,
...
wanted to compare the different mean weights of each genus between plots? (Ignoring `plot_type` for simplicity).
