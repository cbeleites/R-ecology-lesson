...
This provides an overview of all columns/variables grouped by data type/class:
`character` and `numeric`. In contrast to going into RStudio's `Environment`
pane, and clicking the round `>` button left of the `Data` object you want to
inspect, `skim()` also shows some basic statistics for each column.
...
*Pipes* are a better option. They are a recent addition to R and let you take
