...
In the previous lesson, we used `spread()` to transform `surveys` to find the
...
```{r, output-table, message=FALSE, warning=FALSE, echo=FALSE, purl=FALSE}
library(tidyverse)
library(knitr)
...
```{r output-ggplot, message=FALSE, warning=FALSE, echo=FALSE, purl=FALSE}
library(ggplot2)
read_csv("data/surveys_complete.csv") %>%
  count(year, genus, sex) %>% 
  ggplot(mapping = aes(x = year, y = n, color = sex)) +
